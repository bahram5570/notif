    ...
    <div className="w-full grid grid-cols-12 gap-x-5 gap-y-6">
      {updatedCategories.map((item, index) => (
        <CustomLink href={`/category/${item.slug}`} style={{ gridColumn: `span ${item.cols}` }} key={index}>
          <div className="h-[60px] rounded-xl flex items-center justify-between gap-1 px-5 border-[1px] border-impo_Surface_OutlineVariant">
            <CustomTypography fontSize="Title_Small" className="!text-impo_Surface_InverseSurface">
              {item.title}
            </CustomTypography>

            <ArrowIcon className="w-4 fill-impo_Neutral_OnBackground" />
          </div>
        </CustomLink>
      ))}
    </div>
