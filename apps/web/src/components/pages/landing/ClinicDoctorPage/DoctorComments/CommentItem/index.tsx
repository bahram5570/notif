...
const CommentItem = ({ userCommentPositive, userCommentNegitive, userRate = 4, userText }: CommentItemProps) => {
    return <div className='rounded-[16px] px-3 py-[16px]' style={{ backgroundColor: COLORS_LIST.Surface_SurfaceVariant }}>
        <StarRating rating={userRate} />
        <CustomTypography fontSize="Body_Large" tagType='h6' className='mt-4'>{userText}</CustomTypography>
        ...
        <div className='grid items-center gap-y-2 gap-x-2 m-[12px]'>
            {userCommentPositive.map((item, index) => (
                <div className='flex gap-2 items-center' key={index}>
                    ...
                    <CustomTypography fontSize="Body_Small" tagType='span' className='text-right' style={{ color: COLORS_LIST.Neutral_OnBackground }}>
                        {item}
                    </CustomTypography>
                </div>
            ))}
