  ...
  return (
    <div className="w-full  mx-auto rounded-xl  overflow-hidden" id="">
      <table className="w-full border-collapse text-center font-sans text-[1.45rem] mobile:text-[1.25rem]">
        <thead>
          <tr className=" text-white " style={{ backgroundColor: COLORS_LIST.Primary_Primary }}>
            {TITLE_LIST.map((title, index) => {
              return (
                <th
                  className="py-3 px-4 uppercase tracking-wider font-bold"
                  key={index}
                  style={{ border: `1px solid ${COLORS_LIST.Pink_300}` }}
                >
        ...
        <tbody>
          {data.map((item, index) => (
            <tr key={index} style={{ backgroundColor: index % 2 === 0 ? COLORS_LIST.Pink_50 : COLORS_LIST.Pink_100 }}>
              <td className="py-3 px-4 " style={{ border: `1px solid ${COLORS_LIST.Pink_300}` }}>
