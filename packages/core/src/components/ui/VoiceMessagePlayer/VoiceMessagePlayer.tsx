  ...
  return (
    <div className="flex items-center gap-2 bg-impo_Grey_100 rounded-xl">
      <button
        disabled={!audioSrc}
        onClick={togglePlay}
        className={`rounded-full w-12 h-12 flex justify-center items-center  ${isMan ? 'bg-impo_PrimaryMan_PrimaryContainerMan' : 'bg-impo_Primary_PrimaryContainer'}`}
      >
        {isPlaying ? (
          <PauseIcon
            className={`w-5 h-auto  ${isMan ? 'fill-impo_PrimaryMan_PrimaryMan' : 'fill-impo_Primary_Primary'}`}
          />
        ) : (
          <PlayIcon
            className={`w-5 h-auto  ${isMan ? 'fill-impo_PrimaryMan_PrimaryMan' : 'fill-impo_Primary_Primary'}`}
